...
JSON_HEIGHT = 10           # Minecraft font "height"
...
for sheet_index in range(sheets):
    start = sheet_index * max_per_sheet
    end = min((sheet_index + 1) * max_per_sheet, len(emoji_list))
    subset = emoji_list[start:end]
...
pack_meta = {
    "pack": {
        ...
        "description": f"{PACK_NAME} (auto-generated)"
    }
}
